	...
	onModuleInit() {
		...
		// `);
			...
			INSERT INTO user_stats (user_id, total_games, wins, losses,
				 average_score, average_game_duration_in_seconde, updated_at)
			VALUES (@user_id, @total_games, @wins, @losses, @average_score, @average_game_duration_in_seconde, CURRENT_TIMESTAMP)
			ON CONFLICT(user_id) DO UPDATE SET total_games = EXCLUDED.total_games, wins = EXCLUDED.wins, 
