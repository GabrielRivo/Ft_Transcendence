...
class Ball {
	...
	constructor(model?: Mesh) {
		const white: Color4 = new Color4(1, 1, 1, 1);
		this.model = model ?? MeshBuilder.CreateSphere('ball', { diameter: this.diameter });

		this.startDirection();

		const material = new StandardMaterial('ballmat', Services.Scene);
	...
	startDirection() {
		const angle: number = (Math.random() * Math.PI) / 2 - Math.PI / 4; // + ou moin PI pour le sens
	...
	move() {
		...
		const ray = new Ray(this.position, this.direction, distance + this.diameter / 2);
		let overlapping = Services.Collision!.isInside(this.position, 'paddle');

		let hit = Services.Scene!.pickWithRay(
			ray,
			(mesh) => mesh !== this.model && mesh.isPickable && !overlapping.find((m) => m === mesh),
		);
		...
		while (hit && hit.pickedMesh) {
			...
			hit = Services.Scene!.pickWithRay(
				ray,
				(mesh) => mesh !== this.model && mesh.isPickable && !overlapping?.find((m) => m === mesh),
			);
		}
	...
	hit(hitInfo: PickingInfo) {
		const pickedMesh: OwnedMesh<any> = hitInfo.pickedMesh as OwnedMesh<any>;
	...
	findRadialImpact(collidedMesh: OwnedMesh): PickingInfo | null {
		...
		const ray: Ray = new Ray(this.position, this.direction, radius + 1);
		const accuracy = 8;
		...
		for (let i = 0; i <= accuracy; i++) {
			const step: number = i / accuracy;
			const angle: number = -Math.PI / 2 + step * Math.PI;

			rayDirection = this.direction
				.scale(Math.cos(angle))
				.add(left.scale(Math.sin(angle)))
				.normalize();
			ray.direction = rayDirection;
			const hit = Services.Scene!.pickWithRay(
				ray,
				(mesh) => mesh !== this.model && mesh.isPickable && !overlapping?.find((m) => m === mesh),
			);
	...
	findRadialImpactDebug(): PickingInfo | null {
		...
		//white : origin
		const sphere = MeshBuilder.CreateSphere('debugSphere', { diameter: 0.03 }, Services.Scene);
		sphere.isPickable = false;
		sphere.position = ray.origin;
		const debugMat = new StandardMaterial('debugMat', Services.Scene);
		...
		for (let i = 0; i <= accuracy; i++) {
			...
			if (hit && hit.pickedMesh && hit.distance < shortestDist) {
				...
				const mat = new StandardMaterial('debugMat2', Services.Scene);
		...
		const sphere2 = MeshBuilder.CreateSphere('debugSphere', { diameter: 0.03 }, Services.Scene);
		sphere2.isPickable = false;
		sphere2.position = impact?.pickedPoint!.add(impact.getNormal(true)!.scale(0.1));
		const debugMat2 = new StandardMaterial('debugMat', Services.Scene);
	...
	bounce(hitInfo: PickingInfo) {
		const normal: Vector3 | null = hitInfo.getNormal(true);
		if (!normal) console.log('No normal found for bounce!');
		else this.setDir(MathUtils.reflectVector(this.direction, normal));
	}
...
function pickingInfoClone(info: PickingInfo): PickingInfo | null {
	const clone = Object.assign(new PickingInfo(), info);

	clone.pickedPoint = info.pickedPoint?.clone() || null;
	if (!clone.pickedPoint || !info.ray) return null;
