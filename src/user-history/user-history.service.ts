...
const addMatchHistoryStatement: string = `INSERT INTO game_history (game_id, player1_id, player2_id, score_player1,
												score_player2, winner_id, duration_seconds, game_type, tournament_id) 
...
const updateUserGlobalStats : string = `UPDATE user_stats 
										...
										tournaments_won = tournaments_won + @won_tournament, 
...
onModuleInit(){
	...
	add_match_to_history(game_id: number, player1_id: number, player2_id: number, score_player1: number,
								score_player2: number, winner_id: number, duration_seconds: number, game_type: string,
										tournament_id: number | null = null, is_final: boolean = false) {
			...
			if (safeTournamentId) {
				const stmt = this.db.prepare(`INSERT OR IGNORE INTO tournament_players (tournament_id, user_id) VALUES (?, ?)`);
			...
			const matchData = { game_id, player1_id, player2_id, score_player1, score_player2, winner_id, duration_seconds, game_type, tournament_id };

			return (this.runMatchTransaction(matchData, p1Stats, p2Stats, is_final), { message: "Match registered" });
